...
if ( ! function_exists('_form_common'))
{
	 ...
	 */
	function _form_common($type='text', $data='', $value='', $label='', $extra='', $tooltip = '',$coll='col-xs-4')
	{
		...
		// If tooltip is empty at this point, try to grab it from the $data array
		if (empty($coll) && is_array($data) && isset($data['coll']))
		{
			$coll = $data['coll'];
			unset($data['coll']);
		}
...
<div class="form-group {$error}">
	<label class="{$coll} control-label" for="{$defaults['name']}">{$label}</label>
	<div class="col-xs-5">
		 <input {$output} {$extra} />
		{$tooltip}
	</div>
...
if ( ! function_exists('form_dropdown'))
{
	 ...
	 * @return string A string with the formatted input element, label tag and wrapping divs.
	 */
	function form_dropdown($data, $options=array(), $selected=array(), $label='', $extra='', $tooltip = '',$collabel='col-xs-4')
	{
		if (! is_array($data))
		{
...
<div class="form-group {$error}">
	<label class="{$collabel} control-label" for="{$data['id']}">{$label}</label>
	<div class="col-xs-5">
		 <select {$output} {$extra}>
			{$options_vals}
		</select>
