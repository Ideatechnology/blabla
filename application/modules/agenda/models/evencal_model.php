...
class Evencal_model extends CI_Model {
	...
	function getDateEvent($year, $month,$day){
		$year  = ($month < 10 && strlen($month) == 1) ? "$year-0$month" : "$year-$month";
		
		$query = $this->db->select('tgl_agenda')->from('agenda')->like('tgl_agenda', $year, 'after')->get();
		
		if($query->num_rows() > 0){
			$data = array();
			foreach($query->result_array() as $row){
			$jml=$this->db->query("select * from bf_agenda where tgl_agenda='".$row['tgl_agenda']."'")->num_rows();
		$data[(int) end(explode('-',$row['tgl_agenda']))] = $jml;
			}
			return $data;
		}else{
			return false;
	...
	function getEvent($year, $month, $day){
		...
		$query = $this->db->select('id as id, time_start as time, judul_agenda as event,time_ends,author,isi_agenda,tgl_agenda')->order_by('time_start')->get_where('agenda', array('tgl_agenda' => $year));
