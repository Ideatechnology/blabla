...
class Plugin extends Admin_Controller{
	...
	public function __construct() 
	{
        ...
        #load js dan css
		Assets::clear_cache();
		Assets::add_module_js('slide', array('slide.js'));
		
        #load model
        $this->load->model("slide_model",NULL,true);

        #load lang 
        $this->lang->load("slide");

		#load template
   		Template::set('toolbar_title',"Slide");
	...
	{
 	   $this->auth->restrict('Slide.Plugin.View');
		...
		if (isset($_POST['publish']))
		{
			...
			if (is_array($checked) && count($checked))
			{
				...
				foreach ($checked as $pid)
				{
					...
					$result = $this->db->query("update bf_slide set deleted=0 where id='".$pid."'");
		...
		if (isset($_POST['unpublish']))
		{
			...
			if (is_array($checked) && count($checked))
			{
				...
				foreach ($checked as $pid)
				{
					...
					$result = $this->db->query("update bf_slide set deleted=1 where id='".$pid."'");
       ...
       // Deleting anything?
		if (isset($_POST['delete']))
		{
			...
			if (is_array($checked) && count($checked))
			{
				...
				foreach ($checked as $pid)
				{
					...
					$result = $this->slide_model->delete($pid);
				}
				...
				{
					Template::set_message(lang('slide_delete_failure') . $this->slide_model->error, 'danger');
				}
			}
		}
		// Simpan Slide
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Slide.Plugin.Create');
			if ($insert_id = $this->save_slide())
			{
				
				Template::set_message(lang('slide_create_success'), 'success');
				redirect(SITE_AREA .'/plugin/slide');
			}
			else
			{
				Template::set_message(lang('slide_create_failure') . $this->slide_model->error, 'danger');
			}
		}

	   #load path slide
 	   $ListView = $this->slide_model->select()->result();
	...
	public function edit(){
       ...
       $this->auth->restrict('Slide.Plugin.Edit');
       ...
       if (empty($id))
		...
		{
			$this->auth->restrict('Slide.Plugin.Edit');
			if ($insert_id = $this->save_slide("update",$id))
			{
			...
			{
				Template::set_message(lang('slide_create_failure') . $this->slide_model->error, 'error');
			}
		}
	   
	   #parsing into TextFields
	   $value = $this->slide_model->find($id);
	 ...
	 */
	private function save_slide($type='insert', $id=0)
	{
		if ($type == 'update')
		{
			$_POST['id'] = $id;
		...
		$data['link']        = $this->input->post('slide_link');
                $data['keterangan']        = $this->input->post('slide_keterangan');
		...
		if($upload==FALSE):
		   ...
		   $attachment = $files['file_name'];
            ...
            $image_dengan_300 = $files["file_path"].$files["raw_name"]."_768_300".$files;
		...
		if ($type == 'insert')
		{
			$id = $this->slide_model->insert($data);
