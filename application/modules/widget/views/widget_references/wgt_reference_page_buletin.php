<?php
  if(isset($sort_no)){
     $data = $this->db->query('select a.*, b.title, b.id as post_id, b.jml_buletin from bf_widget a, bf_widget_buletin b
					where
...
?>
<?php echo form_open($this->uri->uri_string(),"id='pagebuletin' class='form-vertical'"); ?>
...
<input name="sort_no_identity" type="hidden" value="<?php echo (isset($sort_no) ? $sort_no : '');?>" />
			...
			<?php echo form_label('Jumlah', ' Jumlah', array('class' => 'control-label') ); ?>
				<div class='controls'>
					 <input name="jml_buletin" type="text" value="<?php echo isset($data->jml_buletin) ? $data->jml_buletin :'';?>" style="width:25px;" /> <em>Contoh : 10</em>
				</div>
			</div>
