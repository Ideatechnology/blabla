...
class Settings extends Admin_Controller
{
	...
	private function save_settings($extended_settings=array())
	{
		...
		$filelogo = "";
		$pathfile = "./bonfire/modules/settings/images/";
		$config['upload_path'] = $pathfile;
		$config['allowed_types'] = 'jpg|jpeg|png|gif';
		$config["encrypt_name"]=true;
		$this->load->library('upload', $config);
		$this->upload->initialize($config);
		$filelogo= $this->input->post("logo_hidden");
		if(@$_FILES['logo']['name'] != '') {
		...
		if($upload==FALSE):
			...
			if($type == 'update'){
				@unlink($pathfile.$edit->image_data);

			}
		$filelogo = $attachment1;
		endif;
		...
		$data = array(
			array('name' => 'site.kode_desa', 'value' => $this->input->post('kode_desa')),
			array('name' => 'site.title', 'value' => $this->input->post('title')),
			array('name' => 'site.system_email', 'value' => $this->input->post('system_email')),
			array('name' => 'site.alamat', 'value' => $this->input->post('alamat')),
			array('name' => 'site.telpfax', 'value' => $this->input->post('telpfax')),
			//array('name' => 'site.status', 'value' => $this->input->post('status')),
			array('name' => 'site.list_limit', 'value' => $this->input->post('list_limit')),
			array('name' => 'site.facebook', 'value' => $this->input->post('facebook')),
			array('name' => 'site.twitter', 'value' => $this->input->post('twitter')),
			array('name' => 'site.googleplus', 'value' => $this->input->post('googleplus')),
			array('name' => 'site.cse', 'value' => $this->input->post('cse')),
			array('name' => 'site.keyword', 'value' => $this->input->post('seo_keyword')),
			array('name' => 'site.description', 'value' => $this->input->post('seo_description')),
			array("name"=>'site.logo','value'=>$filelogo),
